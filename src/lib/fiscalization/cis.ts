  ...
  return create({ version: '1.0', encoding: 'UTF-8' })
    ...
    .ele('tns:Racun')
      ...
      .ele('tns:OznRac')
        ...
        .ele('tns:OznUr').txt(racun.brOznUr).up()
      .up()
      .ele('tns:IznosUkupno').txt(racun.iznosUkupno).up()
      .ele('tns:NacinPlac').txt(racun.nacinPlac).up()
      .ele('tns:OibOper').txt(racun.oibOper).up()
      .ele('tns:ZastKodIzdav').txt(racun.zki ?? '').up()
      .ele('tns:NakDost').txt('false').up()
      .ele('tns:UspostavljenoOsiguranje').txt(racun.uspostavljenoOsiguranje ? 'true' : 'false').up()
    .up()
    .up()
